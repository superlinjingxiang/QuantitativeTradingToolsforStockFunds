    ...
    <main class="main-content">
      ...
      <section class="workspace-grid">
        ...
        <aside class="insight-stack">
          ...
          <section class="info-card"><h2>决策证据</h2><div class="info-grid"><b>执行状态</b><span class="tag warn">{{ data?.decision?.execution_status || '仅研究观察' }}</span><b>门禁信号</b><span>{{ signalLabel(data?.decision?.final_signal || operation.final_signal) }}</span><b>置信度</b><span>{{ data?.decision?.confidence || operation.confidence || '--' }}</span><b>回测证据</b><span>{{ backtest.total_return || '--' }} · 回撤 {{ backtest.max_drawdown || '--' }} · Sharpe {{ backtest.sharpe_ratio || '--' }}</span><b>滚动前推</b><span>{{ backtest.walk_forward_consistency || '--' }}</span></div></section>
        </aside>
      </section>
