  ...
  <div class="app-shell">
    ...
    <main class="main-content">
      ...
      <section class="kpi-grid">
        ...
        <article class="kpi accent-amber"><small>回测状态</small><strong>{{ backtest.total_return || '--' }}</strong><span>{{ backtest.status || '样本外验证待加载' }} · 交易 {{ backtest.trade_count || '--' }} · 胜率 {{ backtest.win_rate || '--' }}</span></article>
      </section>
      ...
      <section class="workspace-grid">
        ...
        <aside class="insight-stack">
          <section class="info-card"><h2>当前策略</h2><div class="info-grid"> <template v-for="row in rows(strategy, [['模式','mode_label'],['策略','strategy_id'],['窗口','horizon_label'],['资产','asset_scope'],['样本','sample_count'],['模型','model_version']])" :key="row.label"><b>{{ row.label }}</b><span>{{ row.value }}</span></template></div></section>
          ...
          <section class="info-card"><h2>操作与风险</h2><div class="info-grid"><b>策略建议</b><strong :class="signalClass(operation.final_signal)">{{ signalLabel(operation.final_signal) }}</strong><b>等级</b><span>{{ operation.grade || '--' }} {{ operation.grade_description || '' }}</span><b>账户建议</b><strong>{{ accountAssessment.accountAdvice || '--' }}</strong><b>建议金额</b><span>{{ accountAssessment.suggestedAmount || '--' }}</span><b>建议份额</b><span>{{ accountAssessment.suggestedQuantity || '--' }}</span><b>不交易原因</b><span>{{ operation.abstain_reason || '无' }}</span></div></section>
          <section class="info-card"><h2>决策证据</h2><div class="info-grid"><b>执行状态</b><span class="tag warn">{{ data?.decision?.execution_status || '仅研究观察' }}</span><b>门禁信号</b><span>{{ signalLabel(data?.decision?.final_signal || operation.final_signal) }}</span><b>置信度</b><span>{{ data?.decision?.confidence || operation.confidence || '--' }}</span><b>回测证据</b><span>{{ backtest.total_return || '--' }} · 回撤 {{ backtest.max_drawdown || '--' }}</span></div></section>
        </aside>
      </section>
