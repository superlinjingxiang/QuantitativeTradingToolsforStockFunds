...
function evidenceSummary(values: unknown, fallback: string, limit = 3) {
    ...
    <main class="main-content">
      ...
      <section class="workspace-grid">
        ...
        <aside class="insight-stack">
          ...
          <section class="info-card"><h2>操作与风险</h2><div class="info-grid"><b>策略建议</b><strong :class="signalClass(operation.final_signal)">{{ signalLabel(operation.final_signal) }}</strong><b>等级</b><span>{{ operation.grade || '--' }} {{ operation.grade_description || '' }}</span><b>仓位上限</b><span>{{ operation.target_position_limit || '0.0%' }}</span><b>支持依据</b><span>{{ evidenceSummary(operation.positive_drivers, '暂无足够正向证据', 2) }}</span><b>主要风险</b><span>{{ evidenceSummary(operation.negative_drivers, '暂无额外风险说明', 3) }}</span><b>失效条件</b><span>{{ evidenceSummary(operation.exit_or_invalidation_conditions, '--', 2) }}</span><b>账户建议</b><strong>{{ accountAssessment.accountAdvice || '--' }}</strong><b>不交易原因</b><span>{{ operation.abstain_reason || '无' }}</span></div></section>
