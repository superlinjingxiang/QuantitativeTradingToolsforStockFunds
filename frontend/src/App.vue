    ...
    <main class="main-content">
      ...
      <section class="workspace-grid">
        ...
        <aside class="insight-stack">
          ...
          <section class="info-card"><h2>操作与风险</h2><div class="info-grid"><b>策略建议</b><strong :class="signalClass(operation.final_signal)">{{ signalLabel(operation.final_signal) }}</strong><b>等级</b><span>{{ operation.grade || '--' }} {{ operation.grade_description || '' }}</span><b>仓位上限</b><span>{{ operation.target_position_limit || '0.0%' }}</span><b>支持依据</b><span>{{ operation.positive_drivers?.[1] || operation.positive_drivers?.[0] || '暂无足够正向证据' }}</span><b>主要风险</b><span>{{ operation.negative_drivers?.[0] || '暂无额外风险说明' }}</span><b>失效条件</b><span>{{ operation.exit_or_invalidation_conditions?.[0] || '--' }}</span><b>账户建议</b><strong>{{ accountAssessment.accountAdvice || '--' }}</strong><b>不交易原因</b><span>{{ operation.abstain_reason || '无' }}</span></div></section>
          <section class="info-card"><h2>决策证据</h2><div class="info-grid"><b>执行状态</b><span class="tag warn">{{ data?.decision?.readiness || '仅研究观察' }}</span><b>门禁信号</b><span>{{ signalLabel(data?.decision?.final_signal || operation.final_signal) }}</span><b>置信度</b><span>{{ data?.decision?.confidence || '--' }}</span><b>门禁汇总</b><span>{{ data?.decision?.gate_summary || '--' }}</span><b>阻断原因</b><span>{{ data?.decision?.blocking_reasons?.[0] || '无' }}</span><b>回测证据</b><span>{{ backtest.total_return || '--' }} · 回撤 {{ backtest.max_drawdown || '--' }} · 平均暴露 {{ backtest.average_position_fraction || '--' }}</span><b>执行真实性</b><span>{{ backtest.execution_realism || '--' }}</span><b>滚动前推</b><span>{{ backtest.walk_forward_consistency || '--' }}</span><b>成本压力</b><span>{{ backtest.cost_stress || '--' }}</span></div></section>
        </aside>
      </section>
