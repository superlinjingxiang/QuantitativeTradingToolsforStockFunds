...
| V1.1     | 2026-06-30 | 明确策略决策中枢与盈利验证闭环为产品核心，新增TASK-026。 |
| V1.2     | 2026-06-30 | 明确下一阶段优先做策略盈利证据、可信回测和模拟盘验证。   |
...
本阶段进一步明确：GUI不是产品目的本身，而是数据查看、策略解释、回测复核和模拟验证的工作台。产品核心是“策略决策中枢”：对当前股票、ETF或基金整合行情、因子、预测、风险、历史证据和模拟盘表现，给出可解释的操作建议、仓位边界、失效条件和是否进入后续API执行候选。所有“能赚钱”的判断必须落到可复现证据上，而不是界面展示或主观描述。

当前优先级进一步收敛为三件事：第一，策略是否在扣除成本、考虑规则约束和容量后仍有正期望；第二，回测是否可信，是否避免未来数据泄漏、过拟合和参数偶然性；第三，模拟盘是否能验证信号到成交的真实偏差、稳定性和风险控制。Electron或前后端分离可以提升体验，但不得优先于这三类证据。
...
| US-09    | 作为用户，我输入股票或基金代码后，系统能计算当前走势并给出带历史赚钱证据的建议。 | P0         |
...
18. 用户输入或选择当前标的，也可以选择自选池、ETF池或基金池。

19. 系统加载行情、市场状态、因子、规则、成本、持仓和模拟盘上下文。

20. 策略决策中枢统一调用策略评分、预测引擎、风险引擎、回测证据和模拟盘证据。

21. 系统输出最终建议：买入候选、持有、减仓、卖出、观察或不交易，并显示置信度、仓位上限、有效期和失效条件。

22. 系统同时展示该类信号在历史回测、样本外、滚动验证和模拟盘中的净收益、回撤、胜率、校准和基准比较。

23. 只有当证据满足研究门槛、风险门槛和模拟盘偏差门槛时，标的或策略才可进入未来API执行候选；首版不得直接真实下单。
...
| FR-021   | 策略决策中枢 | 汇总行情、策略、预测、风险、回测和模拟盘证据，给出当前标的建议和执行候选状态。 | P0         |
...
策略决策中枢是后续优化的核心模块。它不新增一个绕过规则的“荐股按钮”，而是把已实现的数据、策略、预测、风险、回测、模拟盘和审计能力组合成统一决策报告。

| **输入** | 当前标的或标的池、实时/历史行情、市场状态、因子快照、交易规则、交易成本、当前持仓、回测证据和模拟盘证据。 |
|----------|----------------------------------------------------------------------------------------------------------------|
| **输出** | 最终建议、置信度、收益/回撤区间、建议仓位上限、支持因素、反对因素、失效条件、证据摘要和执行候选状态。         |
| **门槛** | 必须通过数据健康、样本外验证、成本/滑点压力测试、风险预算、概率校准、模拟盘偏差和审计完整性检查。             |
| **边界** | 可以为未来券商API执行准备候选信号和风控字段，但在未完成合规、凭据、安全和人工确认前不得提交真实订单。         |
...
| 阶段6：策略决策中枢 | 统一当前标的建议、赚钱证据、风险边界和执行候选状态。        | 通过TASK-026验收门槛。               |
| 阶段7：盈利验证深化 | 策略验证实验室、样本外/滚动回测、过拟合诊断和模拟盘偏差报告。 | 通过TASK-027与EPV门槛。              |
| 阶段8：体验与扩展 | Electron/Web前端、更多策略、服务端或合规券商接口。           | 不降低验证门槛，单独立项和合规评估。 |
