...
| US-09 策略盈利证据深化 | FR-022、FR-013至FR-015、FR-020、FR-021、C-006 | EPV-001至EPV-006 | TASK-027、TASK-034、TASK-035 | `tests/unit/test_profit_validation_strategy.py`覆盖ETF盈利验证算法、周期参数、每年交易次数约束、最终样本外阈值不泄漏、十标的聚合证据和下跌市场不通过；`tests/gui/test_app_shell.py`覆盖策略期限/交易次数控件、回测页盈利验证结果和无模拟盘证据不升级执行候选；`tests/gui/test_chart_workspace.py`覆盖回测曲线按钮、历史最大利润 B/S 标记、交易次数上限和正常显示恢复；`tests/unit/test_ai_hedge_fund_integration.py`覆盖ai-hedge-fund独立研究入口、repo解析、API key预检查和子进程隔离；后续仍需成本/容量压力、参数敏感性、过拟合诊断、模拟盘偏差和DecisionHub EPV门禁集成测试 |
