...
当前价和研究分析使用两条不同刷新链：`/api/quote`只取标准Quote并以2秒TTL缓存，Vue可见窗口每3秒轮询；`/api/analyze`负责历史K线、预测、策略、回测和账户评估，继续使用15秒热缓存。实时价更新不得触发长期历史和回测重算，历史缓存也不得覆盖更晚的实时Quote。

概率预测终点和策略持有窗口是两条独立时间语义。`AnalysisReport.horizon`用于p05/p50/p95终点分布，当前短线为5个交易日、长线为10个交易日；`AnalysisReport.strategy_horizon`用于回测、持有和再平衡，当前分别为21日和126日。前端图表只能按前者延展，策略面板和回测证据使用后者。
...
ETF容量审计属于策略证据服务，不属于前端或账户的独立策略。组合回测保留现金和持仓，只按执行日当前名义金额与目标权重之间的差额交易；容量服务消费该调仓事件中的实际权重变化，不重新推导理想订单。实际换手与费用以结构化`PortfolioStrategyEvidence`进入DecisionHub；手动账户只能用计划资金重新评估该报告，不能重算买卖方向或绕过最终门禁。
