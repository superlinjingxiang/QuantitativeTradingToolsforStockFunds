...
- ETF V10差额调仓与多切片验证：`docs/research/SHORT_TERM_STRATEGY_VALIDATION_V10_TURNOVER_2026-07-20.md`
- ETF V11双周期确认候选（拒绝晋级）：`docs/research/SHORT_TERM_STRATEGY_V11_DUAL_HORIZON_CANDIDATE_2026-07-20.md`
- ETF V13逆方差降仓候选（回测门槛通过，等待模拟盘）：`docs/research/SHORT_TERM_STRATEGY_V13_INVERSE_VARIANCE_CANDIDATE_2026-07-20.md`
- ETF V13多起点影子持有段验证（历史影子通过，仍待模拟盘）：`docs/research/SHORT_TERM_STRATEGY_V13_SHADOW_EPISODE_VALIDATION_2026-07-20.md`

## 当前状态

规格基线：V1.0，日期为2026年6月28日。

最新工程进度快照见：`docs/PROJECT_STATUS_2026-07-20.md`。该文档汇总 FastAPI/Vue/Redis 迁移、行情与交互改造、荐股池、账户输入联动、短线策略 V2-V7、V8 候选否决、V9 固定十 ETF 候选、V10 差额调仓会计修正、V11-V13 顺序候选审计、预测校准 V2、组合证据与 DecisionHub 联动、ETF 容量/冲击成本和 T+0/T+1 门禁，以及当前仍未解除的交易边界。`docs/PROJECT_STATUS_2026-07-17.md` 保留为历史快照。

截至 2026 年 7 月 20 日，固定十 ETF 轮动候选已升级为 V10 差额调仓会计：只交易当前持仓与目标权重之间的差额，显式记录真实换手和累计费用，并由容量 V2 复用实际交易权重变化。100 万元参考资金的容量状态为 `PASS`，2% 目标参与率支持资金约 349.95 万元；但最后 20%、25%、30% 时间留出的完整滚动折分别只有 1、1、2 个，组合状态继续为 `WATCH`。容量通过不能替代模拟盘和盘口成交证据。A 股个股多组独立验证仍为 0/10 PASS；平台继续定位为研究、回测和模拟验证工具，不连接真实券商下单，也不承诺未来收益。

同日完成 V11 双周期确认候选审计：要求 252 日和 126 日动量同时为正虽然降低了部分回撤，但开发段净收益由 +46.91% 降至 +27.44%、Sharpe 由 1.10 降至 0.84、平均调仓换手由 35.06% 升至 46.44%，最后 30% 留出相对等权基准为 -2.03%。候选被拒绝，生产默认和右侧决策模块继续使用 V10 `SINGLE_HORIZON`。
...
2026年7月14日完成盈利验证策略 V7 阶段改造：在 V6 的次日开盘、T+1 和涨跌停约束上修正日线止损峰值时点，并为 A 股个股增加按时点计算的沪深300市场环境门槛。代码改动见 `docs/CHANGELOG_2026-07-14.md`，V7 参数、三套互不重叠的 A 股池以及 ETF/混合池结果见 `docs/research/SHORT_TERM_STRATEGY_VALIDATION_V7_2026-07-14.md`，V6 结果保留为执行时序对照。

当前十 ETF 和混合十标的实验都只有 2/10 PASS；正式、确认、影子三套十 A 股池均为 0/10 PASS。正式池平均收益为 +1.05%，确认池为 -7.93%，影子池为 +1.23% 但中位数为负。市场环境过滤改善了部分样本，却没有形成跨股票池稳定盈利证据。当前策略只用于研究观察，不代表保证盈利。可使用以下命令复现：
...
2026年7月17日新增固定十 ETF 的 252 日正绝对动量轮动候选验证器。全样本 7 个滚动窗口通过，但最后 25% 时间留出段只有 1 个完整滚动窗口，因此候选仍为 `WATCH`，没有接入真实买卖建议。完整选择过程、失败股票候选、留出结果和风险边界见 `docs/research/SHORT_TERM_STRATEGY_VALIDATION_V9_ETF_ROTATION_2026-07-17.md`。复现命令：

    .\.venv\Scripts\python.exe -m china_quant_platform.strategies.etf_rotation_lab --history-years 9 --oos-fraction 0.25 --compact

V9 结果现已作为附加证据进入 `AnalysisReport`、DecisionHub、右侧四个模块和手动账户评估。组合横截面排名不替代单标的方向概率，`WATCH`、陈旧缓存或未入选状态都不能许可新增仓位；详细契约和账户语义见 `docs/research/ETF_ROTATION_DECISION_INTEGRATION_2026-07-17.md`。

2026年7月20日新增 `etf-capacity-impact-v1`，随后在 V10 中升级为 `etf-capacity-impact-v2`。容量审计只读取每个调仓信号日及以前的成交额，按实际执行权重差额计算买卖名义金额，并用 `15 + 120 × sqrt(ADV参与率)` bp 的透明假设估算往返成本。固定十 ETF 的 T+0/T+1 分类、容量状态、真实换手、累计费用和账户资金场景进入同一 DecisionHub 门禁；历史 V1 方法见 `docs/research/ETF_CAPACITY_IMPACT_VALIDATION_2026-07-20.md`，当前权威结果见 `docs/research/SHORT_TERM_STRATEGY_VALIDATION_V10_TURNOVER_2026-07-20.md`。

同日依次预注册并复核两个ETF动量候选。V11要求252日与126日动量同时为正，因开发段收益、Sharpe、滚动稳定性、换手和容量综合退步被拒绝。V12按经典 prior (2-12) momentum 思路把排名终点改为 `t-21`；其开发段Sharpe和回撤略有改善，但净收益由V10的+46.91%降至+45.52%，45bp压力收益由+44.41%降至+43.07%，未达到预注册门槛，因此没有读取最终时间留出或候选容量。两项实验都只保留为显式研究开关；Vue/Electron、DecisionHub、右侧四模块和手动账户继续使用V10 `SINGLE_HORIZON`。详见 `docs/research/SHORT_TERM_STRATEGY_V11_DUAL_HORIZON_CANDIDATE_2026-07-20.md`、`docs/research/SHORT_TERM_STRATEGY_V12_SKIP_MONTH_PREREGISTRATION_2026-07-20.md` 和 `docs/research/SHORT_TERM_STRATEGY_V12_SKIP_MONTH_CANDIDATE_2026-07-20.md`。

随后预注册 V13 逆方差风险暴露候选。该候选不改变 V10 的排名和执行，只在高波动时把仓位缩放从 `20% / 波动率` 改为 `(20% / 波动率)^2`。固定十 ETF 开发段和三个最终时间留出均通过预注册门槛：全样本净收益 +95.72%、最大回撤 -13.60%、Sharpe 1.223，最后 20%/25%/30% 留出均为正收益和正超额且 Sharpe 高于 V10，100 万元容量为 `PASS`。但留出完整滚动折只有 1/1/2，模拟盘成交偏差仍缺失，因此 V13 只登记为待模拟盘验证的 `RESEARCH_ONLY` 候选；应用和账户继续默认 V10。详见 `docs/research/SHORT_TERM_STRATEGY_V13_INVERSE_VARIANCE_PREREGISTRATION_2026-07-20.md` 和 `docs/research/SHORT_TERM_STRATEGY_V13_INVERSE_VARIANCE_CANDIDATE_2026-07-20.md`。

V13随后完成预注册的多起点影子持有段复核。全样本51个互不重叠调仓段中，V10/V13证券选择一致率为100%，V13在13/14个V10下跌段中减少亏损，平均亏损幅度降低30.41%，上涨收益保留85.87%；最后30%时间段的19个持有段也通过固定门槛。该结果登记为`SHADOW_RESEARCH_PASS`，只增强历史风险控制证据，不替代模拟盘和真实成交验证，也不改变当前V10、单标的预测、右侧操作建议或手动账户。复现命令和完整边界见`docs/research/SHORT_TERM_STRATEGY_V13_SHADOW_EPISODE_VALIDATION_2026-07-20.md`。

同日预测区间模型升级为 `forecast.similar_regime_interval.v2`：21日预测按21日步长统计独立校准时点，并隔离当时尚未兑现的训练标签。两组各十个真实标的均完整取数，平均区间覆盖分别为82.67%和80.40%，三分类Brier分别为0.1938和0.1987。该结果只说明预测校准达到当前研究门槛，不代表交易策略保证盈利。方法和复现命令见 `docs/research/FORECAST_INTERVAL_VALIDATION_V2_2026-07-17.md`。
