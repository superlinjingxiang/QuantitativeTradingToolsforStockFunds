...
最新工程进度快照见：`docs/PROJECT_STATUS_2026-07-17.md`。该文档汇总 FastAPI/Vue/Redis 迁移、行情与交互改造、荐股池、账户输入联动、短线策略 V2-V7、V8 候选否决、V9 固定十 ETF 组合候选、预测校准 V2、组合证据与 DecisionHub 联动，以及当前仍未解除的交易门禁。`docs/PROJECT_STATUS_2026-07-14.md` 保留为历史快照。

截至 2026 年 7 月 17 日，固定十 ETF 轮动候选已经作为附加研究证据接入右侧四个决策模块和手动账户同一决策链，但最后 25% 时间留出仅有 1 个完整滚动窗口，状态仍为 `WATCH`。A 股个股多组独立验证仍为 0/10 PASS；平台继续定位为研究、回测和模拟验证工具，不连接真实券商下单，也不承诺未来收益。
