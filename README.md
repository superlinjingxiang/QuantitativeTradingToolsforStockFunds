...
最新工程进度快照见：`docs/PROJECT_STATUS_2026-07-20.md`。该文档汇总 FastAPI/Vue/Redis 迁移、行情与交互改造、荐股池、账户输入联动、短线策略 V2-V7、V8 候选否决、V9 固定十 ETF 组合候选、预测校准 V2、组合证据与 DecisionHub 联动、ETF 容量/冲击成本和 T+0/T+1 门禁，以及当前仍未解除的交易边界。`docs/PROJECT_STATUS_2026-07-17.md` 保留为历史快照。

截至 2026 年 7 月 20 日，固定十 ETF 轮动候选已增加按时点 20 日平均成交额容量审计、平方根冲击成本和产品 T+0/T+1 分类。100 万元参考资金的容量状态为 `PASS`，2% 目标参与率支持资金约 349.68 万元；但最后 25% 时间留出仍只有 1 个完整滚动窗口，组合状态继续为 `WATCH`。容量通过不能替代模拟盘和盘口成交证据。A 股个股多组独立验证仍为 0/10 PASS；平台继续定位为研究、回测和模拟验证工具，不连接真实券商下单，也不承诺未来收益。
...
2026年7月20日新增 `etf-capacity-impact-v1`。容量审计只读取每个调仓信号日及以前的成交额，按目标权重绝对变化计算买卖名义金额，并用 `15 + 120 × sqrt(ADV参与率)` bp 的透明假设估算往返成本。固定十 ETF 的 T+0/T+1 分类、容量状态和账户资金场景进入同一 DecisionHub 门禁；完整方法和最新真实复跑见 `docs/research/ETF_CAPACITY_IMPACT_VALIDATION_2026-07-20.md`。
