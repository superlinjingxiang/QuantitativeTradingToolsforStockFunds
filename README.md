...
2026年7月13日完成盈利验证策略 V6 阶段改造：在 V5 目标波动、止损和 A 股反追涨基础上，将收盘退出信号改为下一交易日开盘执行，并加入 A 股 T+1、停牌和一字涨跌停阻断。代码改动见 `docs/CHANGELOG_2026-07-13.md`，V6 执行模型、规则依据和三组十标的结果见 `docs/research/SHORT_TERM_STRATEGY_VALIDATION_V6_2026-07-13.md`，V5 结果保留为对照。

当前十 ETF 和混合十标的实验都只有 2/10 PASS；十 A 股在修复理想成交时点后平均最终收益为 -0.14%，且仍为 0/10 PASS。当前策略只用于研究观察，不代表保证盈利。可使用以下命令复现：
