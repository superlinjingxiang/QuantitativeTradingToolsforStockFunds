...
- 2026-07-20 V10证据：修正ETF每次调仓全卖全买的执行会计，改为只交易当前持仓与目标权重差额；固定池和参数未改变。全样本净收益+97.47%、最大回撤-21.54%、平均调仓换手35.73%、累计费用/初始资金1.92%；最后20%/25%/30%留出因完整滚动折仅1/1/2继续WATCH。容量V2使用120个实际交易观测，100万元PASS。换手与费用已进入AnalysisReport、DecisionHub和右侧四模块。
...
## 阶段21——ETF V10差额调仓与真实换手费用

### [x] TASK-049——ETF差额调仓、真实换手费用与多时间切片复测
- 依赖：TASK-045、TASK-047、TASK-048
- 需求：FR-008、FR-009、FR-013至FR-015、FR-020至FR-022、EPV-001至EPV-006
- 核心定位：修复固定十ETF组合在目标不变时仍全卖全买的执行会计偏差，使回测、容量、DecisionHub和界面共用同一份实际交易事实；不得通过最终留出调参提高收益。
- 交付：目标权重差额交易；现金和持仓股数连续会计；调仓目标权重、实际权重变化、换手和费用事件；累计换手/平均调仓换手/累计费用；容量V2实际差额观测；Schema、DecisionHub、右侧四模块和缓存v5；20%/25%/30%时间留出复跑。
- 验收：目标组合未变化时不得产生虚假全仓换手；费用必须与实际交易差额对账；容量不得再使用理想目标变化代替实际执行；旧缓存不得伪装为V10；任何留出切片滚动折少于3时必须保持WATCH。
- 完成证据：固定十ETF 10/10取数成功、1320个共同交易日。全样本净收益+97.47%、最大回撤-21.54%、Sharpe1.06、超额+59.08%、平均调仓换手35.73%、累计费用/初始资金1.92%；20%/25%/30%留出净收益+31.37%/+41.24%/+34.58%，完整滚动折1/1/2，全部WATCH。容量V2共120个实际交易观测且0缺失，100万元最大参与率0.5715%、最大模型成本24.07bp并PASS。320项Python回归、Ruff、mypy、发布审计、Vitest、Vite构建通过；Playwright两条业务用例通过但Windows服务回收仍有编排告警。完整证据见`docs/research/SHORT_TERM_STRATEGY_VALIDATION_V10_TURNOVER_2026-07-20.md`。

## 阶段22——ETF V11双周期确认候选审计

### [x] TASK-050——预注册双周期动量候选并执行拒绝晋级门禁
- 依赖：TASK-045、TASK-047至TASK-049
- 需求：FR-013至FR-015、FR-020至FR-022、EPV-001至EPV-006
- 核心定位：验证252日主趋势叠加126日确认能否降低动量反转风险；开发段、固定池、成本和时间留出必须预先锁定，不得因最终留出结果搜索参数。
- 交付：`SINGLE_HORIZON/DUAL_HORIZON_CONSENSUS`研究枚举；CLI显式选择；紧凑报告配置；双周期正动量、无前视、参数约束和生产默认保护测试；固定十ETF开发段、全样本、20%/25%/30%留出及容量比较。
- 验收：候选只有在开发段收益、Sharpe、滚动稳定性、换手和风险综合不差，并在多个留出切片保持正超额时才可晋级；负结果必须写入ADR和研究报告，生产默认不能静默改变。
- 完成证据：候选开发段收益+27.44%、回撤-11.66%、Sharpe0.84、正收益折2/4、平均调仓换手46.44%；V10基线分别为+46.91%、-21.54%、1.10、4/4和35.06%。候选最后30%留出超额-2.03%，100万元最大参与率1.8749%，2%参与率支持资金约106.67万元。结论为拒绝晋级，默认保持V10 `SINGLE_HORIZON`。325项Python回归、Ruff、变更模块mypy和差异检查通过。完整证据见`docs/research/SHORT_TERM_STRATEGY_V11_DUAL_HORIZON_CANDIDATE_2026-07-20.md`。

## 阶段23——ETF V12跳过最近一月候选审计

### [x] TASK-051——预注册prior (2-12)动量候选并在开发段失败后停止
- 依赖：TASK-045、TASK-047至TASK-050
- 需求：FR-013至FR-015、FR-020至FR-022、EPV-001至EPV-006
- 核心定位：验证跳过最近21个交易日能否降低短期反转影响，同时使用独立预注册和顺序门禁避免查看最终留出后挑选跳过窗口。
- 交付：`SKIP_RECENT_MONTH`研究枚举；固定21日跳过窗口；CLI和紧凑报告配置可见；排名端点、跳过区间隔离、窗口约束和生产默认保护测试；固定十ETF前70%开发段同源对照。
- 验收：候选必须同时不弱于V10的开发段净收益、Sharpe、45bp压力收益、滚动稳定性和换手，并保持回撤门槛；任一门槛失败后不得读取候选最终留出、容量或当前持仓，不得继续搜索其他跳过窗口。
- 完成证据：候选开发段净收益+45.52%、最大回撤-20.49%、Sharpe1.109、平均调仓换手34.45%、45bp压力收益+43.07%；V10基线分别为+46.91%、-21.54%、1.103、35.06%和+44.41%。净收益与压力收益失败，候选停止且未读取最终留出/容量，默认保持V10 `SINGLE_HORIZON`。328项Python回归、Ruff format、Ruff lint、mypy和差异检查通过。完整预注册和结果见`docs/research/SHORT_TERM_STRATEGY_V12_SKIP_MONTH_PREREGISTRATION_2026-07-20.md`与`docs/research/SHORT_TERM_STRATEGY_V12_SKIP_MONTH_CANDIDATE_2026-07-20.md`。

## 阶段24——ETF V13逆方差风险暴露候选

### [x] TASK-052——预注册逆方差降仓并执行开发/最终顺序门禁
- 依赖：TASK-045、TASK-047至TASK-051
- 需求：FR-013至FR-015、FR-020至FR-022、EPV-001至EPV-006
- 核心定位：不改变V10动量排名和执行会计，只验证高波动时用逆方差比逆波动更快降低组合暴露，能否在保留大部分费后收益的同时改善Sharpe和最大回撤；候选通过回测也不得绕过模拟盘。
- 交付：`INVERSE_VOLATILITY/INVERSE_VARIANCE`暴露模型枚举；CLI显式选择；紧凑报告暴露参数；高波动降仓、低波动无杠杆、排名一致、未来隔离和生产默认测试；固定十ETF前70%开发段、全样本、20%/25%/30%留出及容量比较。
- 验收：开发段必须同时通过收益保留、Sharpe提升、回撤改善、压力、滚动折、换手和真实降仓门槛，才允许读取最终证据；最终三个留出必须正收益、正超额、回撤受控且Sharpe不低于V10；通过后仍保持`RESEARCH_ONLY`，直至模拟盘成交偏差和足够滚动折完成。
- 完成证据：开发段净收益+44.87%、最大回撤-13.60%、Sharpe1.223、45bp压力收益+42.49%，11项门槛全部通过。全样本净收益+95.72%、最大回撤-13.60%、Sharpe1.223；20%/25%/30%留出净收益+30.81%/+40.08%/+35.27%，Sharpe1.570/1.633/1.260，全部高于V10且正超额。100万元容量PASS，支持资金约349.95万元；留出完整滚动折仅1/1/2，默认仍为V10。331项Python全量回归、Ruff format、Ruff lint、137个源文件mypy、发布审计和差异检查通过。完整证据见`docs/research/SHORT_TERM_STRATEGY_V13_INVERSE_VARIANCE_PREREGISTRATION_2026-07-20.md`与`docs/research/SHORT_TERM_STRATEGY_V13_INVERSE_VARIANCE_CANDIDATE_2026-07-20.md`。

## 阶段25——ETF V13多起点影子持有段复核

### [x] TASK-053——预注册并验证V10/V13互不重叠调仓持有段
- 依赖：TASK-049、TASK-052
- 需求：FR-013至FR-015、FR-020至FR-022、EPV-001至EPV-006
- 核心定位：在不改变排名、调仓、费用或生产默认的前提下，将V10/V13按调仓执行日划分为连续、互不重叠的影子持有段，检查逆方差降仓的下行改善是否覆盖多个起始日期，而不是只依赖单一完整回测。
- 交付：影子持有段、结构化比较报告和固定门槛模型；V10/V13配置差异保护；分段收益与完整回测对账；CLI `--shadow-compare-v13`；全样本和最后20%/25%/30%真实复核。
- 验收：证券选择与排名必须100%一致；持有段不得重叠或读取后续数据；全样本和最后30%必须分别满足预注册样本、下跌改善、亏损幅度、上涨保留、收益、Sharpe和回撤门槛；即使通过也不得绕过模拟盘。
- 完成证据：固定十ETF 10/10取数成功、1320个共同交易日。全样本51段、14个V10下跌段，选择一致率100%，V13在13/14下跌段改善，平均亏损减少30.41%，上涨收益保留85.87%，最大回撤由-21.54%改善至-13.60%。最后30%有19段、6个下跌段，平均亏损减少36.66%，上涨保留85.57%，净收益+35.27%、超额+13.24%，状态PASS；最后20%/25%得到同方向结果。结论为`SHADOW_RESEARCH_PASS`，生产默认仍为V10，账户和四模块不改用V13。Ruff、mypy、23项聚焦测试、335项Python全量回归、发布审计和差异检查通过。完整证据见`docs/research/SHORT_TERM_STRATEGY_V13_SHADOW_EPISODE_PREREGISTRATION_2026-07-20.md`与`docs/research/SHORT_TERM_STRATEGY_V13_SHADOW_EPISODE_VALIDATION_2026-07-20.md`。

## 阶段26——当前价与研究分析刷新分流

### [x] TASK-054——独立实时报价轮询与日线增量更新
- 依赖：TASK-005、TASK-010、TASK-040
- 核心定位：当前价必须及时变化，但不得每3秒重拉长期历史或重跑策略；日线收盘价和缓存结果不得冒充实时Quote。
- 交付：FastAPI `/api/quote`；2秒Quote热缓存；Vue每3秒可见窗口轮询；跨标的请求取消；失败保留上次报价；实时/延迟/休市状态；日线当天点增量更新。
- 验收：价格卡片读取`Quote.latest_price`和`previous_close`；历史分析仍走原缓存；旧请求不覆盖新标的；接口失败不清空已显示价格；交易时段超过90秒标记延迟。
- 完成证据：2026-07-20真实联网请求`SSE:513300`在14:36:04返回2.559、来源Eastmoney、接收延迟约1秒，而旧界面日线值为2.567；Ruff、137个源文件mypy、Python全量336项、Vitest 3项和Vite生产构建通过；Playwright 2条页面用例通过，保留已知Windows服务回收超时说明。

### [x] TASK-055——预测终点分布图语义校正
- 依赖：TASK-054
- 核心定位：预测图必须准确展示后端终点收益分位数，同时不能把终点区间插值伪装成逐日价格预测。
- 交付：p05/p50/p95终点价格与收益率标注；非逐日预测提示；移除逐日圆点；右侧标签空间；终点投影纯函数及测试。
- 验收：预测从最后真实价起步；终点严格等于`latest_price * (1 + quantile)`；周期读取后端预测窗口；提示和标签不被裁切。
- 完成证据：截图2.556起点与-12.0%/+1.1%/+9.5%终点对账得到2.249/2.584/2.799；Vitest 6项、Vite生产构建通过，Playwright页面已显示21日终点非逐日预测提示。

## 阶段27——预测有效期选择与策略窗口分离

### [x] TASK-056——多周期预测校准并发布5日短线终点
- 依赖：TASK-046、TASK-055
- 核心定位：预测终点必须由模型的样本外校准决定，不能继续复用策略持有期；缩短预测图不得改变原策略、交易次数或回测窗口。
- 交付：短线5日、长线10日预测周期策略；`AnalysisReport.strategy_horizon`；右侧预测期；Vue旧缓存失效；短线5日终点图；22标的1/3/5/10/21日研究报告。
- 验收：短线策略面板仍显示21 bars但预测面板和图表显示5个交易日；长线策略窗口仍为126 bars但预测期为10日；终点价格继续严格等于当前价乘收益分位数；旧21日缓存不能冒充新结果。
- 完成证据：2026-07-24多资产池22/22取数成功；5日平均独立样本250.7、覆盖87.69%、下破5.81%、三分类Brier 0.1913、终点误差2.63%；21日对应59.5、81.58%、8.79%、0.1933和6.12%。真实513300响应对账为策略21 bars、预测5日；338项Python测试、Ruff、mypy、7项Vue单测和Vite构建通过，Playwright 2条页面业务断言通过。完整结果见`docs/research/FORECAST_HORIZON_VALIDATION_2026-07-24.md`。

## 阶段28——Vue回测曲线状态与买卖点对齐

### [x] TASK-057——统一回测入口并修复交易日期坐标映射
- 依赖：TASK-055、TASK-056
- 核心定位：顶部回测按钮、图表信号复选框、后端回测开关和ECharts买卖点必须使用同一状态，交易日期必须与当前可见K线横轴严格对齐。
- 交付：前端回测状态同步；后端`SIGNALS` overlay归一化；`trade_date`到ISO行情轴映射；B/S标记纯函数与浏览器交互测试。
- 验收：点击“回测曲线”后按钮显示“正常显示”、复选框勾选且买卖点出现；再次点击或取消复选框恢复普通模式；不可见日期不得生成错位标记。
- 完成证据：真实`SSE:513300`三个月日线返回73点、24个买卖标记，首个买点2026-04-07、最后卖点2026-07-21；339项Python全量回归、Ruff、138个源文件mypy、9项Vue单测、Vite构建和Playwright两条页面场景通过。

## 阶段29——手动账户与生产策略闭环

### [x] TASK-058——账户持仓上下文接入同一策略建议链
- 依赖：TASK-047、TASK-048、TASK-054、TASK-057
- 核心定位：用户填写计划资金、现金、持仓数量、成本价和风险偏好后，必须基于当前短线/长线策略及DecisionHub门禁生成个性化仓位建议；账户模块不得另算买卖方向。
- 交付：空账户识别；证券代码归一化持久化；搜索/切换标的账户隔离；风险偏好和回撤约束的仓位折算；顶部账户建议；右侧策略硬上限、当前仓位、个性化目标、盈亏、金额、份额和原因；缓存Schema v8。
- 验收：默认风险偏好不能让空账户显示为已连接；账户策略信号和门禁信号必须分别等于当前分析和DecisionHub结果；个性化目标不得超过策略硬上限；保存后顶部和右侧立即更新；不同标的不得串用账户。
- 完成证据：343项Python测试、13项Vue单测、Vite生产构建、2条Playwright页面场景、Ruff和139个源文件mypy全部通过；详细变更见`docs/CHANGELOG_2026-07-24.md`。
